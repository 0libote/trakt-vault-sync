 ...
 * without changing what `{{title}}` means everywhere else. When the template
 ...
 *
 * Distinct Trakt items can share a title and year. Without disambiguation,
 * the second
 ...
 *
 *  - **Tier 0** (default): the configured filename
 *  - **Tier 1** (only when originalTitle differs from title): inject the
 *    original title alongside the current title.
...
export function disambiguatedFilename(
  ...
  // Skip tier 1 when the original and current titles are identical.
